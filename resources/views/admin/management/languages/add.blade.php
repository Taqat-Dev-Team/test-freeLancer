...
<script>
    $(document).ready(function () {
        ...
        $('#addLanguageModal').on('hidden.bs.modal', function () {
            $('#addLanguageForm')[0].reset(); // Reset form fields
        ...
        $('#addLanguageForm').on('submit', function (e) {
            ...
            $.ajax({
                url: "{{ route('admin.management.languages.store') }}",
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
                success: function (response) {
                    ...
                    toastr.success(response.message || 'Language added successfully!');
                ...
                error: function (xhr) {
                    if (xhr.status === 422) {
                        ...
                        $.each(errors, function (key, value) {
                            let inputField = $('#addLanguageForm').find(`[name="${key}"]`);
